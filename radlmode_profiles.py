...
class radlmode:
    ...
    def __init__(self,Rmaj,A,delta,kappa,B0,Ip,fgr,fuel,impurity,T0,alpha_T,alpha_n,alpha_imp,T_sep,amp_n,amp_imp,b_T,b_n,b_imp,c_T,c_n,c_imp,H,xi,f_ash,f_LH,scalinglaw):
    ...
    def get_noble_lrad(self,t,impurity):
        ...
        if np.any(t < 0.05) or np.any(t>100): #FIXME: changed lower t bound to 0.05 (from 0.1)
    ...
    def calc_lmode(self):
        ...
        self.n20_r = ( self.amp_n*(1-self.rho**self.c_n)**self.alpha_n  + self.b_n )
        ...
        self.nimp = self.n20                                       
        self.nimp_r = ( self.amp_imp*(1-self.rho**self.c_imp)**self.alpha_imp  + self.b_imp )
        dum = np.average(self.nimp_r)
        self.nimp_r *= self.nimp / dum

        # temperature profile.
        # separatrix temperature is set at 0.1 keV
        self.T_r = (self.T0-self.T_sep) * (1-self.rho**self.c_T)**self.alpha_T + self.b_T #FIXME changed separatrix temp to T_sep 
...
def simple_plot(in_dict,numtrials,xaxis,zaxis,plot_cs = False,contourplot = False):
        ...
        for imp in impurity:          # Scan over each value entered in impurity
            for sl in scalinglaws:    # Scan over each value entered in scalinglaws    
                ...
                for i in np.arange(numtrials):              # Scan over every trial
                    ...
                    T_sep,amp_n,amp_imp,b_T,b_n,b_imp,c_T,c_n,c_imp = var_dict['T_sep'][i],var_dict['amp_n'][i],var_dict['amp_imp'][i],var_dict['b_T'][i],var_dict['b_n'][i],var_dict['b_imp'][i],var_dict['c_T'][i],var_dict['c_n'][i],var_dict['c_imp'][i]
                    ...
                    ps = radlmode(Rmaj,A,delta,kappa,B0,Ip,fgr,fuel,imp,T0,alpha_T,alpha_n,alpha_imp,T_sep,amp_n,amp_imp,b_T,b_n,b_imp,c_T,c_n,c_imp,H,xi,f_ash,f_LH,sl)
                    ps.calc_lmode()
    ...
    label_dict = {'Rmaj':'$R_{maj}$ (m)','A':'$A$','delta':'$\delta$','kappa':'$\kappa$','B0':'$B_0$ (T)','Ip':'$I_p$ (MA)',\
                  'fgr':'$f_{Gr}$','T0':'$T_0$ (keV)','alpha_T':'$\\alpha_T$','alpha_n':'$\\alpha_n$','alpha_imp':'$\\alpha_{imp}$',\
                      'T_sep':'$T_{sep}$','amp_n':'amp$_n$','amp_imp':'amp$_{imp}$','b_T':'$b_T$','b_n':'$b_n$','b_imp':'$b_{imp}$','c_T':'$c_T$','c_n':'$c_n$','c_imp':'$c_{imp}$',\
                      'H':'$H$','xi':'$\\xi$','f_ash':'$f_{ash}$','f_LH':'$f_{LH}$','Pfus/volume':'$P_{fus}/volume$ (MW/m$^3$)',\
                          'Pfus':'$P_{fus}$ (MW)','Palpha':'$P_{\\alpha}$ (MW)','P_LH':'$P_{LH}$ (MW)','Prad':'$P_{rad}$','Pext':'$P_{ext}$ (MW)',\
    ...
    if contourplot:
        ...
        for i in np.arange(numtrials):
            for j in np.arange(numtrials):
                ...
                T_sep,amp_n,amp_imp,b_T,b_n,b_imp,c_T,c_n,c_imp = var_matrix_dict['T_sep'][i][j],var_matrix_dict['amp_n'][i][j],var_matrix_dict['amp_imp'][i][j],var_matrix_dict['b_T'][i][j],var_matrix_dict['b_n'][i][j],var_matrix_dict['b_imp'][i][j],var_matrix_dict['c_T'][i][j],var_matrix_dict['c_n'][i][j],var_matrix_dict['c_imp'][i][j]
        ...
        CS = ax1.contourf(X1,X2,zfield,levels = 50, cmap = cm.winter)
        CS2 = ax1.contour(X1,X2,zfield,levels = 50,cmap = cm.GnBu)
